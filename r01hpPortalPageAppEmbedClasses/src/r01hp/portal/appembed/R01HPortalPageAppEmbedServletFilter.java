 ...
 *
 * [JAVA WEB APP FILTER]: The {@link R01HPortalPageAppEmbedServletFilter} is used as a servlet filter in any java web app
 * 	             		  The architecture is:
 * 									    |
 ...
 * 	              						|
 * 	              						| /{appUrl}?R01HPortal={portal}&R01HPage={page}&R01HLang={lang}
 * 	              					    |
 *                     [R01HPortalAppIncludeServletFilter]  <--- 1.- retrieves the app html (filter.doChain() to the java web app)
 *                     			  [java web app]				 2.- retrieves the container page from the local filesystem using the R01HPortal / R01HPage params
 ...
 *               ... just configure the {@link R01HPortalPageAppEmbedServletFilter} at the [java web app]'s web.xml file
 *
 ...
 *                         	  		     | /{portal}-{page}/{lang}/{appUrl}
 *                         			     |
 *                                [apache web server]			<--- mod_rewrite is used to rewite the url and proxy it to the portal app server (R01H)
 *                                       |
 *                                       | /r01hpProxyWar/{appServerProxyName}/{appURL}?R01HPortal={portal}&R01HPage={page}&R01HLang={lang}
 *                                       |
 *                         [R01HPortalAppIncludeServletFilter]  <--- 1.- retrieves the app's url (paths trim the /r01hpProxyWar/{appServerProxyName} request url part)
 ...
 *                               		 |	                         2.- retrieves the container page from the local filesystem using the R01HPortal / R01HPage params
 *                                       |							 3.- "injects" the app html into the portal page container visual area
 *                                       |
 ...
 *        |-----/someWar/xx--->{rewrite}                                              [R01HAppContainerPageIncludeServletFilter]                       |
 *        |                        |->/r01hpProxyWar/someWar/xx?R01HPortal={}&R0HPage={}              |                                                |
 *        |                        |          |->{WLPROXY}---------/r01hpProxyWar/someWar/xx--------->|                                                |
 ...
 *        |-----/someWar/xx--->{rewrite}                                             [R01HAppContainerPageIncludeServletFilter]
 *        |                        |->/someWar/xx?R01HPortal={}&R01HPage={}                           |
 *        |                        |                                                   {parse the portal page shtml and find
 *        |                        |                                                    the include where dynamic content will
 *        |                        |                                                    be injected}
 *        |                        |                                                                 |
...
@Slf4j
@Accessors(prefix="_")
public class R01HPortalPageAppEmbedServletFilter
  implements Filter {
/////////////////////////////////////////////////////////////////////////////////////////
//  FIELDS
/////////////////////////////////////////////////////////////////////////////////////////
	 ...
	 * Filter help page
	 */
	private final R01HPortalPageEmbedServletFilterHelp _helpRenderer;
	/**
	 * User Agent parser
	 */
...
/////////////////////////////////////////////////////////////////////////////////////////	
	...
	private R01HPortalPageLoaderConfig _pageLoaderConfig;
...
///////////////////////////////////////////////////////////////////////////////////////////
	...
	public R01HPortalPageAppEmbedServletFilter(final R01HPortalPageAppEmbedServletFilterConfig config,
											   final R01HPortalPageLoaderConfig pageProviderConfig,final R01HPortalPageManagerConfig pageManagerConfig,
	...
	public R01HPortalPageAppEmbedServletFilter(final R01HPortalPageAppEmbedServletFilterConfig config,
											   final R01HPortalPageLoaderConfig pageProviderConfig) {
		this(config,
			 pageProviderConfig,new R01HPortalPageManagerConfig());
	}
	public R01HPortalPageAppEmbedServletFilter(final R01HPortalPageAppEmbedServletFilterConfig config) {
		this(config,
			 new R01HPortalPageLoaderConfigForFileSystemImpl(),new R01HPortalPageManagerConfig());
	}
	/**
	...
	public R01HPortalPageAppEmbedServletFilter() {
		...
		if (!props.existsPropertiesFile()) {
			...
			_pageLoaderConfig = new R01HPortalPageLoaderConfigForFileSystemImpl();
		...
		// props from config (can be overridden with web.xml info: see init() method)
			...
			// c) page loader
			_pageLoaderConfig = R01HPortalPageLoaderConfigBase.createFrom(props);
...
///////////////////////////////////////////////////////////////////////////////////////////
	...
	public void init(final FilterConfig filterConfig) {
		...
		// some help
		log.warn("{}",R01HPortalPageAppEmbedServletFilterHelp.filterConfigHelp());
			...
			if (appAndComponent.getAppCode() == null || appAndComponent.getAppComponent() == null) {
				...
				if (props.existsPropertiesFile()) {
					...
					_pageManagerConfig = _pageManagerConfig != null
												...
												: otherPageMgerConfig;
					R01HPortalPageLoaderConfigForFileSystemImpl otherPageProviderConfig = new R01HPortalPageLoaderConfigForFileSystemImpl(props);
		...
		// b) page manager
		R01HPortalPageManagerConfig otherPageMgrConfig = R01HPortalPageAppEmbedServletFilterConfigLoader.loadPortalPageManagerConfigFrom(filterConfig);
		_pageManagerConfig = _pageManagerConfig != null 
									? _pageManagerConfig.cloneOverriddenWith(otherPageMgrConfig)
									: otherPageMgrConfig;
		R01HPortalPageLoaderConfig otherPageLoaderConfig = R01HPortalPageAppEmbedServletFilterConfigLoader.loadPortalPageLoaderConfigFrom(filterConfig);
		...
		// some help
		log.warn(R01HPortalPageAppEmbedServletFilterHelp.filterDebugHelp(_config.isRequestDebuggingGloballyEnabled(),
		...
		try {
			_pageManager = new R01HPortalPageManager(_pageManagerConfig,
													 new R01HPortalPageProvider(_pageManagerConfig,
															 					_pageLoaderConfig));
...
/////////////////////////////////////////////////////////////////////////////////////////
	...
	public void doFilter(final ServletRequest request,final ServletResponse response,
        ...
        Path appModulePath = reqUri.startsWith("/r01hpProxyWar")
        								? Path.from(reqUri.split("/")[2])		// ie: /r01hpProxyWar/foo/baz --> returns foo
        								: Path.from(reqUri.split("/")[1]);		// ie: /foo/baz               --> return foo

    	// ================ METRICS ======================
    	// start metrics
    	R01HPortalPageAppEmbedMetricsContext metricsCtx = null;
    	try {
...
/////////////////////////////////////////////////////////////////////////////////////////
    private int _executeFilter(final HttpServletRequest realHttpReq,
    	...
    	R01HUserAgentData userAgent = ((R01HUserAgentParser)_userAgentParser).from(realHttpReq);
        ...
        if (log.isDebugEnabled()) log.debug("[{}][{}]: INIT app include ip: {} (R01HPortal={}, R01HPage={}, R01HLang={})",
        									ctx.getRequestedUrlPath(),t0,ctx.getClientIp(),ctx.getPortalId(),ctx.getPageId(),ctx.getLang());


        // If the R01HNoPortal param is true, no templating is done
        ...
        if (ctx.isIncludeInAppContainerPageDisabled()) {
        	...
        	R01HFakeStatusCodeCapturingResponseWrapper fakeHttpResponse = new R01HFakeStatusCodeCapturingResponseWrapper(realHttpResp);
        ...
        // The app server's response is included into an app container portal page
	        ...
	        R01HFakeServletResponseWrapper fakeHttpResponse = new R01HFakeServletResponseWrapper(realHttpResp,
	        																					 filterLogger);
        ...
        if (log.isDebugEnabled()) log.debug("[{}][{}]: END app include ip: {} (R01HPortal={}, R01HPage={}, R01HLang={}) > response code={}",
    ...
    private static boolean _isHelpPageReq(final HttpServletRequest realHttpReq) {
    	...
    	return realHttpReq.getQueryString() != null
    	    && realHttpReq.getQueryString().toUpperCase().contains("R01HPHELP=TRUE");	// the r01hHelp=true param is received as request param
    }
}