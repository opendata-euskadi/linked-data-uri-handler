...
//  CONSTRUCTOR
/////////////////////////////////////////////////////////////////////////////////////////
	public R01HPortalPageAppEmbedMetricsConfig() {
		this(false);
	}
	public R01HPortalPageAppEmbedMetricsConfig(final boolean enabled) {
		_enabled = enabled;
	...
	public R01HPortalPageAppEmbedMetricsConfig(final XMLPropertiesForAppComponent props) {
		...
		if (_enabled && slf4jReporterEnabled) {
			TimeLapse reportEvery = props.propertyAt("portalpageappembedfilter/metrics/slf4jReporter/@reportEvery")
										 .asTimeLapse("30s");
			_slf4jReporterConfig = new Slf4jReporterConfig(true,reportEvery,"r01hp.portalpageappembedfilter");
		} else {
			_slf4jReporterConfig = new Slf4jReporterConfig(false,null,null);
		}
		// jmx reporter
	...
	public CharSequence debugInfo() {
		...
		if (_enabled) {
			if (this.isConsoleReporterEnabled()) {
				...
				dbg.append(Strings.customized("\n\t[console reporter]: NOT available (disabled at r01hp.portalpageappembedfilter.properties.xml config file)"));
			}
			if (this.isSlf4jReporterEnabled()) {
				...
				dbg.append(Strings.customized("\n\t[Slf4j reporter]: NOT available (disabled at r01hp.portalpageappembedfilter.properties.xml config file)"));
			}
			if (this.isJMXReporterEnabled()) {
				dbg.append("\n\t[jmx reporter]: enabled");
			} else {
				dbg.append("\n\t[jmx reporter]: NOT available (disabled at r01hp.portalpageappembedfilter.properties.xml config file)");
			}
			if (this.areRESTServicesEnabled()) {
				...
				dbg.append("\n\t[rest services] are NOT available (disabled at r01hp.portalpageappembedfilter.properties.xml config file)");
			}
		} else {
...
/////////////////////////////////////////////////////////////////////////////////////////
	...
	public class RESTServicesConfig 
		...
		@Getter private final Url _metricsUrl = Url.from("/r01hpProxyWar/r01hMetricsRestServicesServlet/metrics");
		@Getter private final Url _healthCheckUrl = Url.from("/r01hpProxyWar/r01hMetricsRestServicesServlet/healthcheck");
		@Getter private final Url _threadsUrl = Url.from("/r01hpProxyWar/r01hMetricsRestServicesServlet/threads");
		@Getter private final Url _pingUrl = Url.from("/r01hpProxyWar/r01hMetricsRestServicesServlet/ping");
